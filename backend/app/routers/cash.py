...
from ..schemas.cash import CashResponse, CashCreate, CashDelete, CashUpdate
...
# Caja - CASH
#
@router.get("/api/cash/", response_model=List[CashResponse], tags=["Cash"])
...
@router.post("/api/cash/", response_model=CashResponse, tags=["Cash"])
def create_a_cash(cash: CashCreate,
                  db: Session = Depends(get_db), 
                  current_user: User = Depends(get_current_active_user)):
    return create_cash(db=db, cash=cash, current_user=current_user)


@router.get("/api/cash/{cash_id}", response_model=CashResponse, tags=["Cash"])
...
@router.patch("/api/cash", response_model=CashResponse,
              tags=["Cash"])
def update_cashes(cash: CashUpdate,
                  db: Session = Depends(get_db),
                  current_user: User = Depends(get_current_active_user)):
    db_client = update_cash(db, cash, current_user)
