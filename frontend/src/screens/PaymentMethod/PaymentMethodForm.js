...
const PaymentMethodForm = React.forwardRef((props, ref) => {
	...
	const getData = (id) => {      		
	  ...
	  const updateData = (values) => {      		  
		DataService.update(endpoint, values)
		  .then(response => {        			
			openNoticeBox("Notice", t("record_updated_successfully", {"table": t("payment_method_table_title")}))
			setData(emptyData)
		  })
		  .catch(e => {  			  			
			openNoticeBox("Error", `Code: ${e.response.status} Message: ${e.response.statusText}`)			
	  ...
	  const addData = (values) => {      		  
		DataService.create(endpoint, values)
		  .then(response => {        			
			openNoticeBox("Notice", t("record_created_successfully", {"table": t("payment_method_table_title")}))
			setData(emptyData)
		  })
		  .catch(e => {  			  
			openNoticeBox("Error", `Code: ${e.response.status} Message: ${e.response.statusText}`)   							
